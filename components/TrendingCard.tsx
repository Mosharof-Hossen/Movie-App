...
const TrendingCard = ({ movie, index }: TrendingCardProps) => {
  const { count, movie_id, title, poster_url } = movie as TrendingMovie;
  console.log(poster_url, index)
  return (
    <Link href={`/movies/${movie.movie_id}`} asChild>
      <TouchableOpacity className='w-32 relative pl-5'>
        ...
        <Text className='text-light-300 text-sm font-bold mt-2' numberOfLines={2}>{title}</Text>
